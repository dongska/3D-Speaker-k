...
class BasicBlockERes2Net(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, baseWidth=32, scale=2):
        ...
        width = int(math.floor(planes*(baseWidth/64.0))) # width计算3x3卷积中每个分组输入（=输出）的通道数 在这里等于输出通道数的一半

        # [B,in_planes,F,T] -> [B,width*scale,F/stride,T/stride]
        ...
        # 第二个1x1卷积
        # [B,width*scale,F/stride,T/stride] -> [B,planes*self.expansion,F/stride,T/stride]
