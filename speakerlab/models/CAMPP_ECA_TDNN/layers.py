...
class CAMLayer(nn.Module):
    def __init__(self,
        ...
        # 时间维分组TDNN
        # 定义 time_groups 个独立的 TDNN（Conv1d），每个对对应时间段进行处理
        self.time_groups = time_groups
        self.conv_time_list = nn.ModuleList([
    ...
    def forward(self, x):
        ...
        # 对时间维分组，裁切并对每组执行对应的 TDNN，最后拼接回来
        T = context_time.shape[-1]
        group_len = math.ceil(T / float(self.time_groups))
        outs = []
        for i, conv in enumerate(self.conv_time_list):
            start = i * group_len
            if start >= T:
                break
            end = min((i + 1) * group_len, T)
            seg = context_time[..., start:end]  # [B, bn_channels, L]
            # 若分段长度为0则跳过
            if seg.shape[-1] == 0:
                continue
