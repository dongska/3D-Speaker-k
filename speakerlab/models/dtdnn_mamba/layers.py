...
class DenseTDNNLayer(nn.Module):
    def __init__(self,
        ...
        self.linear1 = nn.Conv1d(in_channels, bn_channels, 1, bias=False) # 使用conv1d做全连接
    ...
    def bn_function(self, x): # BN+ReLU+全连接
    ...
    def forward(self, x):
        if self.training and self.memory_efficient:
            ...
            x = self.bn_function(x) # BN+ReLU+全连接(维度—>bn_channels)
        x = self.linear2(self.nonlinear2(x)) # BN+ReLU+TDNN
        return x
...
class DenseTDNNBlock(nn.ModuleList):
    def __init__(self,
        ...
        for i in range(num_layers):
            layer = DenseTDNNLayer(in_channels=in_channels + i * out_channels, # 每过一层layer拼接上out_channels维的输出
